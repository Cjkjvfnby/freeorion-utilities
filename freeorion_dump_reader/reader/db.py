...
class Database(dict):
            ...
            for section in SECTIONS:
                ...
                for game_id, branches in self.__branches_ends.items():
                    for branch_end in branches:
                        ...
                        branch_line.reverse()
                        self.__branch_lines[(game, section, branch_end['turn_id'])] = tuple(branch_line)

    def get_branch(self, game_id, section, branch_end, start, end):
        key = game_id, section, branch_end
        branch_line = self.__branch_lines[key]
        if start is None:
            start = 1
        else:
    ...
    def get_branch_end(self, game_id):
        print self.__branches_ends[game_id]
